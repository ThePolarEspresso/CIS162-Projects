...
public class CovidDatabaseGUI extends JFrame implements ActionListener{
    ...
    JButton dailyInfections, safe, peakDeaths, topTen; 

    /** Results */
    JTextArea results;

    /** Additional data fields */
    JTextField stateFld, countFld, monthFld, dayFld;
    ...
    public static void main(String args[]){
        ...
        gui.setTitle("Anna's Covid Database");
        gui.pack();
        gui.setVisible(true);
    }
    ...
    /*****************************************************************
     * @param list <CovidEntry> - list to display
     ****************************************************************/ 
    ...
    private void displayPeakDeaths(){
        ...
        if (isValidMonth() && isValidDay()) {
            ...
            results.append("\n\nPeak daily deaths for " + month + "/" + day + "\n");
            
            if (c != null) {
                results.append(c.toString());
            } else {
                results.append("No records found for date");
            }
        }
    ...
    private void displaySafe(){
        if (isValidState()) {
            ArrayList <CovidEntry> list = database.safeToOpen(stateFld.getText());
            if (list == null) {
                results.setText(stateFld.getText().toUpperCase() + " is not yet safe to open");
            } else {
                results.setText(stateFld.getText().toUpperCase() + " is safe to open\n");
                displayRecords(list);
            }
        }
    }
    ...
    private void displayStats(){
        results.setText("As of " + MONTH + "/" + DAY);
        results.append("\nTotal Database Records: " + fmt.format(database.countRecords()));
        results.append("\nTotal U.S. Infections : " + fmt.format(database.getTotalInfections()));
        results.append("\nTotal U.S. Deaths: " + fmt.format(database.getTotalDeaths()));
    ...
    private boolean isValidMinInfections(){
        ...
        try {
            ...
            isValid = false;
            JOptionPane.showMessageDialog(this, "Enter a Minimum value");
        }
    ...
    private boolean isValidState(){
        ...
        if (stateFld.getText().length() == 0) {
            isValid = false;
            JOptionPane.showMessageDialog(this, "Enter a state");
        }
